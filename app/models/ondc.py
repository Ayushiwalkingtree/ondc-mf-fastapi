...
class MfSearchRequestState(Base):
    __tablename__ = 'mf_search_request_states'

    tracking_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    transaction_id: Mapped[str | None] = mapped_column(String(100), index=True, nullable=True)
    message_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    status: Mapped[str] = mapped_column(String(30), index=True, default='initiated')
    request_payload: Mapped[dict] = mapped_column(JSON)
    catalogue: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    session_id: Mapped[str | None] = mapped_column(String(255), index=True, nullable=True)
    subscriber_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    error: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    ...
    __table_args__ = (
        Index('ix_mf_search_state_tracking_status', 'tracking_id', 'status'),
        Index('ix_mf_search_state_active_session', 'session_id', 'status', 'created_at'),
    )
